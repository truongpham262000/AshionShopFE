...
@Component({
  selector: 'ngx-danh-muc-san-pham-create',
  templateUrl: './danh-muc-san-pham-create.component.html',
  styleUrls: ['./danh-muc-san-pham-create.component.scss']
})
export class DanhMucSanPhamCreateComponent implements OnInit {
  constructor(
    private commonService: CommonService,
    private api: AshionShop,
    @Inject(MAT_DIALOG_DATA) public dataRef: { key: string, actionType: number },
    private dialogRef: MatDialogRef<DanhMucSanPhamCreateComponent>
  ) { }

  statusAction = STATUS_ACTION;
  titleHeader: string = "Danh mục sản phẩm";
  ...
  ngOnInit(): void {
    ...
    this.api.categorySelectAll(TYPE_DANHMUC.PRODUCT).subscribe(res => {
  ...
  loadData(): void {
    this.api.categorySelectOne(this.dataRef.key, TYPE_DANHMUC.PRODUCT).subscribe((res) => {
      if (res.code === SUCCESS_RESPONSE) {
  ...
  save(): void {
    this.target.typeDanhMuc = TYPE_DANHMUC.PRODUCT;
