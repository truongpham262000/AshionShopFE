...
import { AshionShop, FooterModel } from '../../../../@core/data/ashion-shop.service';

@Component({
  selector: 'ngx-trang-chu-footer-create',
  templateUrl: './trang-chu-footer-create.component.html',
  styleUrls: ['./trang-chu-footer-create.component.scss']
})
export class TrangChuFooterCreateComponent implements OnInit {
  constructor(
    private commonService: CommonService,
    private api: AshionShop,
    @Inject(MAT_DIALOG_DATA) public dataRef:{ key: string, actionType: number },
    private dialogRef: MatDialogRef<TrangChuFooterCreateComponent>
  ) { }

  statusAction = STATUS_ACTION;
  trangThai = LIST_STATUS;
  titleHeader: string = "Design footer shop";
  target: FooterModel = new FooterModel();
  ...
  ngOnInit(): void {
    if(this.dataRef.actionType !== this.statusAction.create){
      ...
      this.target.logoImage = null;
      this.target.trangThai = this.listStatus[0]['value'];
    }
  }
  
  loadData(): void {
    this.api.footerSelectOne(this.dataRef.key).subscribe((res) => {
  ...
  save(): void {
    ...
    if(this.dataRef.actionType === STATUS_ACTION.create){
      this.api.footerInsert(this.target).subscribe((res) => {
        if(res.code === SUCCESS_RESPONSE){
      ...
      this.api.footerUpdate(this.target).subscribe((res) => {
        if(res.code === SUCCESS_RESPONSE){
          this.dialogRef.close(true);
          this.loadData();
          this.commonService.toastrSuccess(SUCCESS_NOTICE);
  ...
  handleUpload(event: any){
    ...
    reader.onload = () => {
      this.target.logoImage = reader.result.toString();
    };
  }
