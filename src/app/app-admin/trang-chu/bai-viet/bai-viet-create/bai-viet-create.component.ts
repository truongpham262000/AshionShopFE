...
import { AboutModel, AshionShop } from '../../../../@core/data/ashion-shop.service';

@Component({
  selector: 'ngx-bai-viet-create',
...
export class BaiVietCreateComponent implements OnInit{
  constructor(
    ...
    private dialogRef: MatDialogRef<BaiVietCreateComponent>
  ) { }

  statusAction = STATUS_ACTION;
  trangThai = LIST_STATUS;
  titleHeader: string = "bài viết";
  target: AboutModel = new AboutModel();
  ...
  loadData(): void {
    this.api.aboutSelectOne(this.dataRef.key).subscribe((res) => {
  ...
  save(): void {
    ...
    if(this.dataRef.actionType === STATUS_ACTION.create){
      this.api.aboutInsert(this.target).subscribe((res) => {
        if(res.code === SUCCESS_RESPONSE){
      ...
      this.api.aboutUpdate(this.target).subscribe((res) => {
        if(res.code === SUCCESS_RESPONSE){
          this.dialogRef.close(true);
          this.loadData();
          this.commonService.toastrSuccess(SUCCESS_NOTICE);
